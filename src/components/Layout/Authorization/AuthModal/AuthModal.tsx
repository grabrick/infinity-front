...
const AuthModal = ({
  ...
  const onSubmit = (data: any) => {
    handleOnSubmit(isRegister === true ? "registr" : "login", data);
  };

  const onChangeMode = () => {
    setIsRegister(!isRegister);
    reset({ email: "", password: "", confirmPassword: "", firstName: "", lastName: "" });
    setIsStage(0);
  };
  ...
  return (
    <form className={m.container} onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className={m.confirm}>
        {isStage === 0 && isRegister === true && (
          <div className={m.confirmBtn} onClick={() => nextPage()}>
            Продолжить
          </div>
        )}
        {isStage === 1 && isRegister === true && (
        ...
        <span className={m.text}>
          {title === "Регистрация"
            ...
            : "У вас нет аккаунта?"}{" "}
          <span className={m.changeModal} onClick={() => onChangeMode()}>
