...
const SoundPlayer = ({ item }: any) => {
  const [openIds, setOpenIds] = useState<any>([]);

  const handleOpen = (id: any, fileUrl: any, file: any) => {
    setOpenIds((prevIds: any) => {
      ...
      if (existingIndex >= 0) {
        ...
        newIds[existingIndex] = { id, fileUrl, file };
        return newIds;
      }
      return [...prevIds, { id, fileUrl, file  }];
    });
  };
  ...
  const handleFileChange = (event: any, id: any) => {
    ...
    if (file) {
      ...
      handleOpen(id, fileUrl, file);
    }
  };

  const handleDelete = (id: any) => {
    setOpenIds((prevIds: any) =>
      prevIds.filter((openId: any) => openId.id !== id)
    );
  };
  ...
  const getFileById = (id: any) => {
    ...
    return openItem ? { file: openItem.file, fileUrl: openItem.fileUrl } : null;
  };
          ...
          {!isOpen(el.id) ? (
            <div className={m.loadContainer}>
              ...
              <input
                ...
                onChange={(event) => handleFileChange(event, el.id)}
              />
            </div>
          ) : (
            <>
              <div className={m.soundBoard}>
                <h3 className={m.soundName}>{getFileById(el.id)?.file.name}</h3>
                <Player file={getFileById(el.id)} progressTextColor={"rgba(255, 255, 255, 0.7)"} controlTextColor={"#7a90da"} />
              </div>
